...
class TestIngestTakeOnData():
    ...
    @mock.patch("ingest_takeon_data.funk.read_from_s3")
    @mock.patch("ingest_takeon_data.funk.save_data")
    @mock.patch("ingest_takeon_data.funk.send_sns_message")
    ...
    def test_missing_env_var(self):
        ingest_takeon_data.os.environ.pop("in_file_name")
        returned_value = ingest_takeon_data.lambda_handler(
            None, {"aws_request_id": "666"}
        )
        ingest_takeon_data.os.environ["in_file_name"] = "mock-file"

        assert """Key Error""" in returned_value["error"]

    def test_empty_env_var(self):
        ingest_takeon_data.os.environ["in_file_name"] = ""
        returned_value = ingest_takeon_data.lambda_handler(
            None, {"aws_request_id": "666"}
        )
        ingest_takeon_data.os.environ["in_file_name"] = "mock-file"
